...
function getFunc(req, res, next){
	if(isEmpty(req.params.category)){
		return next(findError("CHECKER.2.1.1"));
	}
	else if(!roles_model.hasCategory(req.params.category)){
		return next(findError("CHECKER.2.1.2"));
	}
	else{
		access.get({ category : req.params.category }, function(err, elmt){
			if(err){
				...
				return next(findError("CHECKER.2.1.3"));
			}
			else{
				var elmt2 = [];
...
function postFunc(req, res, next){
	if(isEmpty(req.params.category)){
		return next(findError("CHECKER.2.2.1"));
	}
	else if(!roles_model.hasCategory(req.params.category)){
		return next(findError("CHECKER.2.2.2"));
	}
	else if(isEmpty(req.params.login)){
		return next(findError("CHECKER.2.2.3"));
	}
	else if(isEmpty(req.body.role)){
		return next(findError("CHECKER.2.2.4"));
	}
	else if(!_.contains(roles_model.getRolesInCategory(req.params.category), req.body.role)){
		return next(findError("CHECKER.2.2.5"));
	}
	else{
...
function deleteFunc(req, res, next){
	if(isEmpty(req.params.category)){
		return next(findError("CHECKER.2.3.1"));
	}
	else if(!roles_model.hasCategory(req.params.category)){
		return next(findError("CHECKER.2.3.2"));
	}
	else if(isEmpty(req.params.login)){
		return next(findError("CHECKER.2.3.3"));
	}
	else{
		access.getOne({ login    : req.params.login,
						category : req.params.category },
				     function(err, elmt){
						if(err){
							...
							return next(findError("CHECKER.2.3.4"));
						}
						else{
							roles_model.overwriteRole({ login      : req.params.login,
														removeRole : true,
