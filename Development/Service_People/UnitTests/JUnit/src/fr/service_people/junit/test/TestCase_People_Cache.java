...
/**
 * This class contains the webService client JUnit tests to check if there was no regression in People service
 * <p>
...
public class TestCase_People_Cache extends TestCase_Model
{
	...
	public void testPeopleCache() throws Exception{		
		...
		Person person = null;
		try{
			person=ws_client.createPerson("loginone", "lastnameone", "firstnameone", "M", new Date(), false, "mail@mail.fr", null, "null1.png", "creator");
		...
		try{			
			ws_client.getOnePersonById(person.get_id());
			cache2=ws_client.getCacheInformation();

			ws_client.getAllPeople();
		...
		try{
			ws_client.updatePerson(person.get_id(), "logintwo", "lastnametwo", "firstnametwo", "M", new Date(), false, "mail2@mail.fr", null, "null2.png", "updator");
		...
		try{
			ws_client.deleteOnePerson(person.get_id());
