...
/* 
 * Class 	: TestCase_Contributor_RejectForbiddenLetter
 * Author(s): Zidmann
 ...
 */
public class TestCase_Contributor_RejectForbiddenLetter extends TestCase_Model
{ 
	...
	protected void testGetOneByIdRequestAuxi(String forbiddenLetter) throws Exception{
		...
		try{
			ws_client_contributor_proxy.getOneContributorById("id"+forbiddenLetter);
			assertNotNull(null);
		}
	...
	@Test
	public void testGetOneByLoginRequest() throws Exception{
		for(int i=0; i<this.forbiddenLetters.length; i++){
			testGetOneByLoginRequestAuxi(this.forbiddenLetters[i]);
		}
	}
	protected void testGetOneByLoginRequestAuxi(String forbiddenLetter) throws Exception{
		initSettings();

		try{
			ws_client_contributor_proxy.getOneContributorByLogin("login"+forbiddenLetter);
			assertNotNull(null);
		}
		catch(APIException e){
			checkBasicMessage(ErrorReferential.getErrorByCode("WSCORE-JAR-2"), e.getMsg());
