...
/* 
 * Class 	: TestCase_Group_RejectForbiddenLetter
 * Author(s): Zidmann
 ...
 */
public class TestCase_Group_RejectForbiddenLetter extends TestCase_Model
{ 
	...
	protected void testGetOneByIdRequestAuxi(String forbiddenLetter) throws Exception{
		...
		try{
			ws_client_group_proxy.getOneGroupById("id"+forbiddenLetter);
			assertNotNull(null);
		}
	...
	protected void testGetOneByNameRequestAuxi(String forbiddenLetter) throws Exception{
		...
		try{
			ws_client_group_proxy.getOneGroupByName("name"+forbiddenLetter);
			assertNotNull(null);
		}
		catch(APIException e){
			checkBasicMessage(ErrorReferential.getErrorByCode("WSCORE-JAR-2"), e.getMsg());
	...
	@Test
	public void testGetOneByLoginSearchRequest() throws Exception{
		for(int i=0; i<this.forbiddenLetters.length; i++){
			testGetOneByLoginSearchRequestAuxi(this.forbiddenLetters[i]);
		}
	}
	protected void testGetOneByLoginSearchRequestAuxi(String forbiddenLetter) throws Exception{
		initSettings();

		try{
			ws_client_group_proxy.searchByLogin("login"+forbiddenLetter);
			assertNotNull(null);
		}
		catch(APIException e){
			checkBasicMessage(ErrorReferential.getErrorByCode("WSCORE-JAR-2"), e.getMsg());
