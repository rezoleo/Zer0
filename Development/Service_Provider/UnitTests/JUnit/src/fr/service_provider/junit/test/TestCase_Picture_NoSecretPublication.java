...
/* 
 * Class 	: TestCase_Picture_NoSecretPublication
 * Author(s): Zidmann
 * Function : This class contains the webService client JUnit tests to check if the version (__v), the IP (creatorIP, updatorIP) are not shown when data are extracted from Provider service through Picture service 
 * Version  : 1.0.0 
 * Note		: This Test Case supposes that you started the NodeJS server
 * 			  and removed all the Picture elements in the MongoDB database
 */
